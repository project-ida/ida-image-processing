...
with tifffile.TiffFile(ome_file) as tif:
    ...
    for z, entries in z_layers.items():
        output_folder = input_folder / f"jpg_output_z{z}"
        ...
        for page, base_name in entries:
            try:
                ...
                Image.fromarray(img).save(jpg_path, "JPEG", quality=100)
                print(f"✅ Saved Z{z}: {jpg_path.name}")
            except Exception as e:
                print(f"⚠️ Could not save image {base_name}.jpg: {e}")
